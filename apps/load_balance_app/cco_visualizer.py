...
class CCOVisualizer:
    ...
    def _plot_scenario(self, ax, title: str, ue_data: pd.DataFrame, attached_data: pd.DataFrame):
        """
        A single subplot is generated by this helper method. UEs are colored by their connected site (not individual cell).
        """
        ...
        base_colors = [
            ...
            "cyan", "magenta", "brown", "black", "navy", "darkgreen"
        ]
        color_map = {site: base_colors[i % len(base_colors)] for i, site in enumerate(unique_sites)}

        legend_handles = []

        # Plot UEs per site (color grouped by site)
        for site in unique_sites:
            site_ues = served_ues[served_ues["site_id"] == site]
            color = color_map[site]
            ax.scatter(site_ues[self.COL_LON], site_ues[self.COL_LAT], color=color, s=10, alpha=0.8)
            legend_handles.append(Line2D([0], [0], marker='o', color='w', label=f'UEs ({site})',
