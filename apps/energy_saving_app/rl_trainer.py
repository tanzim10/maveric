...
def run_rl_training(
    ...
    log_dir: str,
    total_timesteps: int,
    continue_from_model: bool = False,
):
    """
    ...
    continue_from_model : bool
        If True and rl_model_path exists, continue training from the existing model.
    """
    ...
    if continue_from_model:
        if os.path.exists(rl_model_path):
            logger.info(f"Loading existing RL agent from {rl_model_path} for further training.")
            model = PPO.load(rl_model_path, env=env, tensorboard_log=log_dir)
        else:
            logger.warning(f"Model path {rl_model_path} does not exist. Starting fresh training of the RL agent.")
            model = PPO("MlpPolicy", env, verbose=1, tensorboard_log=log_dir)
    else:
        logger.info("Starting fresh training of the RL agent.")
        model = PPO("MlpPolicy", env, verbose=1, tensorboard_log=log_dir)
