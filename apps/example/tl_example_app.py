...
from shapely.geometry import Polygon, box, Point # TODO: Check if shapely added to requirements
    ...
    for i, region_idx_list in enumerate(vor.regions):
        ...
        try:
            ...
            if vor_poly.geom_type == 'Polygon':
                ...
                polys_to_use.extend(list(vor_poly.geoms))
            else:
    ...
    for _, ue_row in trafficload_ue_data.iterrows():
        for _, cell_row in site_config_data.iterrows():
            try:
                ...
                if dist_m > 1e-3:
                    rx_power = ref_rx_power - 10 * path_loss_exponent * np.log10(dist_m)
                else:
...
def plot(tick_to_plot: int, results: Dict, site_config_data: pd.DataFrame, spatial_params: Dict):
    ...
    ax.scatter(
        ...
        marker="^", c="red", label="Cell Towers", s=60, zorder=10
    )
    ...
    for i, cell_id in enumerate(unique_serving):
        ...
        if not cell_ues.empty:
            ...
            if label not in labels:
                handles.append(plt.Line2D([0], [0], marker='o', color=cmap(i), linestyle='', ms=6))
                labels.append(label)
    ...
    if not ues_no_serve.empty:
        ...
        if label not in labels:
            handles.append(plt.Line2D([0],[0], marker='x', color='black', linestyle='', ms=6))
            labels.append(label)
