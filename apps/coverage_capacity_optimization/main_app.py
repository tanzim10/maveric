...
class CCOMainApp:
    ...
    def __init__(
        self, topology_path: str, training_data_path: str, prediction_data_path: str, config_path: str, model_id: str
    ):
        """
    ...
    def train_bdt(self, container_name: str = "radp_training") -> Dict:
        ...
        self.bdt_manager = BDTManager(
            topology_path=self.topology_path, training_data_path=self.training_data_path, model_path=model_path
        )

        # Train model
        self.bdt_manager.train(model_id=self.model_id, container_name=container_name)
    ...
    def train_cco(self) -> Dict:
        ...
        # Train model
        training_results = self.trainer.train(num_epochs=20, epsilon=0.1, seed=42)  # Default number of epochs
    ...
    def preprocess_data(self, base_data_dir: str, days: List[int] = None) -> None:
        ...
        preprocessor = UEDataPreprocessor(base_data_dir)  # noqa: F841
...
Examples:
  # Run full pipeline
  python main_app.py --topology data/topology.csv --training data/training.csv --prediction data/prediction.csv
  --config data/config.csv --model-id my_model
  ...
  python main_app.py --preprocess --base-dir data --days 1 2 3
    ...
    # Optional arguments
    parser.add_argument(
        "--container", type=str, default="radp_prod-training-1", help="Docker container name for BDT training"
    )
    parser.add_argument("--epochs", type=int, default=20, help="Number of training epochs")

    # Preprocessing arguments
    parser.add_argument("--preprocess", action="store_true", help="Run data preprocessing only")
    parser.add_argument("--base-dir", type=str, help="Base directory for data preprocessing")
    parser.add_argument("--days", type=int, nargs="+", help="Day numbers to process")
    ...
    try:
        if args.preprocess:
            ...
            if not all(required_args):
                logger.error(
                    "Full pipeline requires --topology, --training, --prediction, --config, and --model-id arguments"
                )
                sys.exit(1)
