...
from api_manager.app_endpoints import AppEndpoints
...
# App-specific validation endpoints
@app.route("/api/apps/validate", methods=["POST"])
def validate_app_config():
    """Validate app-specific configuration parameters"""
    return AppEndpoints.validate_app_config()


@app.route("/api/apps/validate-full", methods=["POST"]) 
def validate_full_app_config():
    """Validate complete app configuration including files"""
    return AppEndpoints.validate_full_app_config()


@app.route("/api/apps/schema/<app_type>", methods=["GET"])
def get_app_schema(app_type: str):
    """Get validation schema for specific app type"""
    request.view_args = {"app_type": app_type}
    return AppEndpoints.get_app_schema()


@app.route("/api/apps/supported", methods=["GET"])
def list_supported_apps():
    """List all supported app types and their descriptions"""
    return AppEndpoints.list_supported_apps()
